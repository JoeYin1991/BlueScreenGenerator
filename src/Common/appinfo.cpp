...
void AppInfo::save()
{
    ...
    if (model.mEmojiType == EEmojiType::Char) {
        ...
        settings.setValue(KEY_EMOJI_PATH, model.mEmojiImgName);
    }
    ...
    settings.setValue(KEY_QRCODE, model.mQrcodeName);
...
void AppInfo::loadConfig()
{
    ...
    model.mEmojiImgName = settings.value(KEY_EMOJI_PATH).toString();
    if (!model.mEmojiImgName.isEmpty()) {
    ...
    model.mQrcodeName = settings.value(KEY_QRCODE).toString();
